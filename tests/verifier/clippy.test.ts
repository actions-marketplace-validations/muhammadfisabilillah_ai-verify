...
import { ClippyVerifier } from "../../src/verifier/clippy.js";
...
async function initRepo(): Promise<string> {
  const dir = mkdtempSync(path.join(tmpdir(), "ai-verify-clippy-"));
  tempDirs.push(dir);
...
describe("ClippyVerifier", () => {
  const verifier = new ClippyVerifier();

  it("has correct id and name", () => {
    expect(verifier.id).toBe("clippy");
    expect(verifier.name).toBe("Clippy");
  });
